...
class SubcategoryService {
  Future<SubcategoryCategory?> fetchSubcategories(
      {required int categoryId}) async {
    final response = await http
        .get(Uri.parse('$baseUrl/view-subcategories/category/$categoryId'));
  ...
  Future<List<ProductModel>> fetchProductsByCategoryAndSubcategory(
      {required int categoryId, required int subcategoryId}) async {
    final String url =
        '$baseUrl/viewproductsbycatsubcat/?category_id=$categoryId&subcategory_id=$subcategoryId';
    print(url);
